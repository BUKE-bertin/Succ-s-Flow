...
export interface AppState {
  ...
  // Chapter Actions
  addChapter: (chapter: Omit<Chapter, 'id' | 'completed'>) => Promise<void>;
  updateChapter: (id: string, chapter: Partial<Chapter>) => Promise<void>;
  toggleChapterCompleted: (id: string) => Promise<void>;
  deleteChapter: (id: string) => Promise<void>;

  // Study Session Actions
  addStudySession: (session: Omit<StudySession, 'id' | 'completed_at'>) => Promise<void>;
  deleteStudySession: (id: string) => Promise<void>;

  // Settings Actions
  updateSettings: (settings: { examDate?: string; dailyStudyGoal?: number }) => Promise<void>;
...
export const useAppStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      chapters: [],
      studySessions: [],
      examDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 14 jours d'ici
      ...
      syncData: async () => {
        ...
        try {
          ...
          const localOnlySubjects = localSubjects.filter(s => s.id.startsWith('local_'));
          for (const sub of localOnlySubjects) {
            const cleanSub = { name: sub.name, color: sub.color, priority: sub.priority, user_id: user.id };
            const { data: insertedSub } = await supabase.from('subjects').insert(cleanSub).select().single();
            if (insertedSub) {
              // Update local chapters that referenced this local subject ID
              const updatedChapters = get().chapters.map(c => 
                c.subject_id === sub.id ? { ...c, subject_id: insertedSub.id } : c
              );
              // Update local sessions
              const updatedSessions = get().studySessions.map(s => 
                s.subject_id === sub.id ? { ...s, subject_id: insertedSub.id } : s
              );
              set({ chapters: updatedChapters, studySessions: updatedSessions });
          ...
          const localOnlyChapters = localChapters.filter(c => c.id.startsWith('local_'));
          ...
          for (const chap of localOnlyChapters) {
            ...
            const { data: insertedChap } = await supabase.from('chapters').insert(cleanChap).select().single();
            if (insertedChap) {
              // Update local sessions referencing this chapter
              const updatedSessions = get().studySessions.map(s => 
                s.chapter_id === chap.id ? { ...s, chapter_id: insertedChap.id } : s
              );
          ...
          const localOnlySessions = localSessions.filter(s => s.id.startsWith('local_'));

          for (const sess of localOnlySessions) {
            const cleanSess = {
              user_id: user.id,
              chapter_id: sess.chapter_id?.startsWith('local_') ? null : sess.chapter_id,
              subject_id: sess.subject_id?.startsWith('local_') ? null : sess.subject_id,
          ...
          if (finalCloudSessions) {
            set({ studySessions: finalCloudSessions.map(s => ({
              id: s.id,
      ...
      addSubject: async (subjectData) => {
        ...
        const newSubject: Subject = { id: localId, ...subjectData };
        ...
        if (user && supabase) {
          try {
            ...
            if (data) {
              ...
              set({
                subjects: get().subjects.map(s => (s.id === localId ? data : s))
              });
            }
          } catch (e) {
      ...
      updateSubject: async (id, updatedFields) => {
        ...
        // Local update
        set({
          subjects: subjects.map(s => (s.id === id ? { ...s, ...updatedFields } : s))
        });
      ...
      addChapter: async (chapterData) => {
        ...
        const localId = generateUUID();
        const newChapter: Chapter = { id: localId, completed: false, ...chapterData };
        ...
        if (user && supabase) {
          try {
            ...
            if (data) {
              set({
                chapters: get().chapters.map(c => (c.id === localId ? data : c))
              });
            }
          } catch (e) {
            console.error('Supabase insert failed. Kept locally.', e);
      ...
      updateChapter: async (id, updatedFields) => {
        ...
        set({
          chapters: chapters.map(c => (c.id === id ? { ...c, ...updatedFields } : c))
        });
      ...
      toggleChapterCompleted: async (id) => {
        ...
        // Update local state
        set({
          chapters: chapters.map(c => (c.id === id ? { ...c, completed: nextCompleted } : c))
        });
      ...
      addStudySession: async (sessionData) => {
        ...
        const newSession: StudySession = { id: localId, completed_at: completedAt, ...sessionData };
        ...
        if (user && supabase) {
          try {
            const { data, error } = await supabase
              ...
              .insert({
                user_id: user.id,
                chapter_id: sessionData.chapter_id?.startsWith('local_') ? null : sessionData.chapter_id,
                subject_id: sessionData.subject_id?.startsWith('local_') ? null : sessionData.subject_id,
            ...
            if (data) {
              set({
                studySessions: get().studySessions.map(s => (s.id === localId ? {
                  id: data.id,
      ...
      updateSettings: async (settings) => {
        ...
        set((state) => ({
          examDate: settings.examDate !== undefined ? settings.examDate : state.examDate,
          dailyStudyGoal: settings.dailyStudyGoal !== undefined ? settings.dailyStudyGoal : state.dailyStudyGoal,
        }));

        if (user && supabase) {
          try {
            const updates: any = {};
            if (settings.examDate !== undefined) updates.exam_date = settings.examDate;
            if (settings.dailyStudyGoal !== undefined) updates.daily_study_goal = settings.dailyStudyGoal;
    ...
    }),
    {
      ...
      merge: (persistedState: any, currentState) => {
        ...
        return {
          ...currentState,
          ...persistedState,
          subjects: Array.isArray(persistedState.subjects) ? persistedState.subjects : [],
          chapters: Array.isArray(persistedState.chapters) ? persistedState.chapters : [],
          studySessions: Array.isArray(persistedState.studySessions) ? persistedState.studySessions : [],
          examDate: (persistedState.examDate && !isNaN(new Date(persistedState.examDate).getTime()))
            ? persistedState.examDate 
            : currentState.examDate,
          dailyStudyGoal: (typeof persistedState.dailyStudyGoal === 'number' && persistedState.dailyStudyGoal > 0)
            ? persistedState.dailyStudyGoal 
            : currentState.dailyStudyGoal,
          theme: ['light', 'dark'].includes(persistedState.theme) ? persistedState.theme : currentState.theme,
        };
      }
    }
  )
