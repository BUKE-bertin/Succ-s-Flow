...
import { FolderPlus, Trash2, Edit2, Check, X, AlertTriangle } from 'lucide-react';
...
export const Subjects: React.FC = () => {
  const { subjects, chapters, addSubject, updateSubject, deleteSubject, theme } = useAppStore();
  ...
  const handleDelete = async (id: string, name: string) => {
    if (window.confirm(`Voulez-vous vraiment supprimer "${name}" ? Cela supprimera également tous ses chapitres liés.`)) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card p-6 h-fit relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
            <h2 className={`text-[15px] font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2.5">
                {PRESETS_COLORS.map((c) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full transition-all duration-250 relative shrink-0 cursor-pointer ${
                      color === c ? 'scale-110 shadow-[0_2px_8px_rgba(0,0,0,0.2)]' : 'hover:scale-110'
                    }`}
                ...
                <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 border-dashed ${isDark ? 'border-zinc-700 hover:border-zinc-500' : 'border-slate-300 hover:border-slate-400'} transition-colors relative overflow-hidden`}>
                  <input
                    type="color"
                    value={color}
              ...
              {isEditing && (
                <button
                  ...
                  className={`p-3 rounded-xl transition-all duration-200 cursor-pointer border ${
                    isDark ? 'bg-white/[0.04] hover:bg-white/[0.08] border-zinc-800 text-zinc-300' : 'bg-slate-50 hover:bg-slate-100 border-slate-200 text-slate-600'
                  }`}
                  title="Annuler"
                >
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {subjects.length === 0 ? (
            <div className={`h-[300px] flex flex-col items-center justify-center border border-dashed rounded-3xl gap-3 ${
              isDark ? 'text-zinc-500 border-zinc-800/60 bg-white/[0.01]' : 'text-slate-400 border-slate-200 bg-slate-50/50'
            }`}>
              <AlertTriangle className={`w-8 h-8 ${isDark ? 'text-zinc-700' : 'text-slate-300'}`} />
              <p className="text-[14px] font-medium">Aucune matière enregistrée.</p>
              <p className="text-[12px]">Remplissez le formulaire à gauche pour commencer.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {subjects.map((sub) => {
                ...
                const completedChaptersCount = subChapters.filter((c) => c.completed).length;
                const progress = totalChaptersCount > 0 ? Math.round((completedChaptersCount / totalChaptersCount) * 100) : 0;
                
                return (
                  <div key={sub.id} className="card p-5 flex flex-col justify-between group">
                    <div>
                      {/* Card Header */}
                      <div className="flex items-start justify-between gap-3 mb-4">
                        <div className="flex items-center gap-2.5 overflow-hidden">
                          <div className="w-3.5 h-3.5 rounded-full shrink-0 shadow-sm" style={{ backgroundColor: sub.color }}></div>
                          <h3 className={`font-bold text-[15px] truncate ${isDark ? 'text-white' : 'text-slate-900'}`} title={sub.name}>{sub.name}</h3>
                        </div>
                      ...
                      <div>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                          sub.priority === 'high' ? 'bg-red-500/10 text-red-600 dark:text-red-400' :
                          sub.priority === 'medium' ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400' :
                          isDark ? 'bg-white/[0.06] text-zinc-400' : 'bg-slate-100 text-slate-500'
                        }`}>
                          Priorité {sub.priority === 'low' ? 'Basse' : sub.priority === 'medium' ? 'Moyenne' : 'Haute'}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="mt-6 space-y-2">
                      <div className="flex justify-between items-center">
                        <span className={`text-[11px] font-medium ${isDark ? 'text-zinc-500' : 'text-slate-500'}`}>Chapitres</span>
                        <span className={`text-[12px] font-bold tabular-nums ${isDark ? 'text-zinc-300' : 'text-slate-700'}`}>
                          {completedChaptersCount} / {totalChaptersCount} <span className={isDark ? 'text-zinc-500' : 'text-slate-400 font-normal'}>({progress}%)</span>
                        </span>
                      </div>
                      <div className={`w-full h-[6px] rounded-full overflow-hidden ${isDark ? 'bg-zinc-800/60' : 'bg-slate-100'}`}>
                        <div
                          className="h-full rounded-full transition-all duration-700 ease-out"
                          style={{ backgroundColor: sub.color, width: `${progress}%` }}
                        ></div>
                      </div>
                    </div>
                  </div>
