...
export const Planner = () => {
  const { subjects, chapters, examDate, toggleChapterCompleted, theme } =
    useAppStore();
  ...
  const averageDailyMinutes = useMemo(() => {
    return schedule.length > 0
      ? Math.round(totalPlannedMinutes / schedule.length)
      : 0;
  ...
  const handleToggleChapter = async (id, completed) => {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          >
            Algorithme de répartition équilibrée de vos révisions pour les{" "}
            {planningDaysCount} prochains jours.
          </p>
        </div>
      </div>
      ...
      {subjects.length === 0 ? (
        ...
        >
          ...
          <p
            className={`text-[13px] ${isDark ? "text-zinc-500" : "text-slate-500"}`}
          >
            Allez dans l'onglet "Matières" pour configurer votre programme.
          </p>
        </div>
        ...
        >
          ...
          <p
            className={`text-[13px] ${isDark ? "text-zinc-500" : "text-slate-500"}`}
          >
            Vous n'avez aucun chapitre en attente. Félicitations !
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div className="card p-5 flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${isDark ? "bg-indigo-500/10 text-indigo-400" : "bg-indigo-50 text-indigo-600"}`}
              >
              ...
              <div>
                ...
                <p
                  className={`text-[20px] font-extrabold tracking-tight mt-0.5 ${isDark ? "text-white" : "text-slate-900"}`}
                >
            ...
            <div className="card p-5 flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${isDark ? "bg-emerald-500/10 text-emerald-400" : "bg-emerald-50 text-emerald-600"}`}
              >
            ...
            <div className="card p-5 flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${isDark ? "bg-purple-500/10 text-purple-400" : "bg-purple-50 text-purple-600"}`}
              >
          ...
          {/* AI Banner Explainer */}
          <div
            className={`p-4 rounded-2xl flex items-start gap-3.5 border ${
              isDark
          ...
          >
            ...
            <div className="space-y-1.5">
              <h3
                className={`text-[14px] font-bold ${isDark ? "text-white" : "text-slate-900"}`}
              >
                Répartition intelligente
              </h3>
              <p
                className={`text-[12px] leading-relaxed ${isDark ? "text-zinc-400" : "text-slate-600"}`}
              >
                L'algorithme trie vos chapitres par priorité puis difficulté. Il
                les distribue ensuite jour par jour en équilibrant la charge
                horaire (bin-packing) pour éviter la fatigue.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4 pt-2">
            <h2
              className={`text-[18px] font-extrabold tracking-tight ${isDark ? "text-white" : "text-slate-900"}`}
            >
              ...
              {schedule.map((day, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <div
                        className={`flex items-center justify-between pb-3.5 border-b ${isDark ? "border-zinc-800/60" : "border-slate-200/80"}`}
                      >
                        <h3
                          className={`font-bold text-[15px] capitalize flex items-center gap-2.5 ${isDark ? "text-white" : "text-slate-900"}`}
                        >
                          <span
                        ...
                        <span
                          className={`text-[11px] font-bold px-2.5 py-1 rounded-lg transition-colors ${
                            day.totalDuration > 180
                              ? "bg-red-500/10 text-red-600 dark:text-red-400"
                              : day.totalDuration > 90
                      ...
                      <div className="mt-4 space-y-2.5">
                        {day.chapters.length === 0 ? (
                          <p
                            className={`text-[12px] italic py-2 ${isDark ? "text-zinc-500" : "text-slate-400"}`}
                          >
                            Journée libre ! Reposez-vous ou avancez sur d'autres
                            matières.
                          </p>
                        ) : (
                          day.chapters.map((chap) => {
                            const sub = subjects.find(
                              (s) => s.id === chap.subject_id,
                            );
                            return (
                              <div
                                key={chap.id}
                              ...
                              >
                                <button
                                  onClick={() =>
                                    handleToggleChapter(chap.id, chap.completed)
                                  }
