...
import { ShieldAlert, Mail, Lock, LogIn, ArrowRight, UserPlus } from 'lucide-react';
...
export const Auth: React.FC = () => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (!isSupabaseConfigured || !supabase) {
      toast.error("Supabase n'est pas configuré. Veuillez utiliser le mode local.");
      return;
    }
    ...
    try {
      if (isSignUp) {
        ...
        toast.success('Compte créé avec succès ! Un e-mail de confirmation vous a été envoyé.');
      } else {
  ...
  const handleGuestMode = () => {
    toast.success('Bienvenue en Mode Local ! Vos données seront sauvegardées sur cet appareil.');
    navigate('/');
  };
        ...
        <div className="relative z-10 space-y-6">
          ...
          {!isSupabaseConfigured && (
            <div className={`border rounded-2xl p-4 flex gap-3 ${
              isDark ? 'bg-amber-500/[0.04] border-amber-500/20 text-zinc-300' : 'bg-amber-50/80 border-amber-200 text-slate-700'
            }`}>
              <ShieldAlert className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" />
              <div className="text-[12px] space-y-1">
                <span className="font-bold text-amber-500 block">Mode Déconnecté Uniquement</span>
                <p>
                  Les variables Supabase ne sont pas configurées. L'application enregistre tout sur votre appareil (LocalStorage).
                </p>
              </div>
            </div>
          )}
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Mail className={`absolute left-4 top-3.5 w-4 h-4 ${isDark ? 'text-zinc-500' : 'text-slate-400'}`} />
                  <input
                    type="email"
                    value={email}
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Lock className={`absolute left-4 top-3.5 w-4 h-4 ${isDark ? 'text-zinc-500' : 'text-slate-400'}`} />
                  <input
                    type="password"
                    value={password}
          ...
          <div className="relative flex items-center justify-center my-4">
            <div className={`border-t w-full ${isDark ? 'border-zinc-800' : 'border-slate-200'}`}></div>
            <span className={`px-3 text-[10px] font-bold uppercase tracking-widest absolute ${
              isDark ? 'bg-[#111113] text-zinc-600' : 'bg-white text-slate-400'
            }`}>OU</span>
          </div>
          ...
          >
            ...
            <ArrowRight className={`w-4 h-4 ${isDark ? 'text-zinc-500' : 'text-slate-400'}`} />
          </button>
        </div>
      </div>
    </div>
