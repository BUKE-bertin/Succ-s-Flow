...
import { BookPlus, Trash2, CheckCircle2, Circle, Eye, EyeOff, AlertCircle } from 'lucide-react';
import { toast } from 'react-hot-toast';

export const Chapters: React.FC = () => {
  const { subjects, chapters, addChapter, toggleChapterCompleted, deleteChapter, theme } = useAppStore();
  ...
  const filteredChapters = chapters.filter(c => {
    const matchesSubject = selectedSubjectId === 'all' || c.subject_id === selectedSubjectId;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div>
        ...
        <p className={`text-[13px] mt-1 ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
          Détaillez vos révisions par matières, estimez leur temps et suivez votre progression.
        </p>
      </div>
      ...
      <div className={`card p-4 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center`}>
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <span className={`text-[11px] font-semibold uppercase tracking-wider mr-2 ${isDark ? 'text-zinc-500' : 'text-slate-500'}`}>
            Filtrer :
          </span>
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-[12px] font-semibold transition-all duration-200 cursor-pointer border ${
              selectedSubjectId === 'all'
                ? isDark ? 'bg-zinc-100 border-zinc-100 text-zinc-900' : 'bg-slate-800 border-slate-800 text-white'
                : isDark ? 'bg-transparent border-zinc-800 text-zinc-400 hover:text-zinc-200 hover:bg-white/[0.04]' : 'bg-transparent border-slate-200 text-slate-500 hover:text-slate-800 hover:bg-slate-50'
            }`}
          >
            Tous
          </button>
          {subjects.map(sub => (
            ...
            >
              <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: sub.color }}></span>
              {sub.name}
            </button>
          ))}
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-[12px] text-indigo-500 font-bold">{duration} min ({Math.round(duration / 60 * 10) / 10}h)</span>
                </div>
                <input
                  type="range"
                  min="15"
                ...
                `}} />
                <div className={`flex justify-between text-[10px] font-semibold px-0.5 ${isDark ? 'text-zinc-600' : 'text-slate-400'}`}>
                  <span>15 min</span>
                  <span>1h</span>
                  <span>2h</span>
                  <span>3h</span>
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          {filteredChapters.length === 0 ? (
            <div className={`h-[300px] flex flex-col items-center justify-center border border-dashed rounded-3xl gap-3 ${
              ...
              <AlertCircle className={`w-8 h-8 ${isDark ? 'text-zinc-700' : 'text-slate-300'}`} />
                ...
                return (
                  <div
                    ...
                    className={`card p-4 flex items-center justify-between group ${
                      chap.completed 
                        ? isDark ? 'opacity-60 bg-zinc-900 border-zinc-800/50' : 'opacity-60 bg-slate-50 border-slate-200/50'
                        : ''
                    }`}
                  >
                    <div className="flex items-center gap-3.5 min-w-0 flex-1">
                      ...
                      <button
                        ...
                        title={chap.completed ? 'Marquer comme non révisé' : 'Marquer comme révisé'}
                      >
                        {chap.completed ? (
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <h4 className={`font-bold text-[14px] md:text-[15px] truncate ${
                            chap.completed 
                              ? isDark ? 'line-through text-zinc-500' : 'line-through text-slate-400' 
                          ...
                          {sub && (
                            <span 
                              className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 border ${
                                isDark ? 'bg-white/[0.04] border-zinc-800' : 'bg-white border-slate-200'
                              }`}
                        ...
                        <div className="flex items-center gap-3 mt-1.5">
                          <span className={`text-[10px] font-bold capitalize ${
                            chap.difficulty === 'hard' ? 'text-red-600 dark:text-red-400' :
                            chap.difficulty === 'medium' ? 'text-amber-600 dark:text-amber-400' :
                          ...
                          <span className={`text-[11px] font-semibold flex items-center gap-1 ${
                            ...
                            <span className="opacity-70">⏱</span> {chap.estimated_duration} min
                          </span>
                        </div>
                      </div>
                    </div>
