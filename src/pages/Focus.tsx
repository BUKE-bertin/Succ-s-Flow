...
import { Play, Pause, RotateCcw, Coffee, Target, Sparkles, BellRing } from 'lucide-react';
...
export const Focus: React.FC = () => {
  ...
  async function handleTimerComplete() {
    ...
    if (mode === 'focus') {
      ...
      toast.success('La pause est finie. C\'est parti pour une nouvelle session !');
  ...
  const currentSubject = currentChapter ? subjects.find(s => s.id === currentChapter.subject_id) : null;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6 lg:col-span-1">
            ...
            <div className="flex flex-col gap-2.5">
              {PRESETS.map((preset, index) => {
                ...
                return (
                  ...
                  >
                    <p className={`text-[14px] font-bold ${isActive ? 'text-white' : isDark ? 'text-zinc-300' : 'text-slate-700'}`}>
                      {preset.label}
                    </p>
                    <p className={`text-[12px] mt-1 font-medium ${isActive ? 'text-indigo-100' : isDark ? 'text-zinc-500' : 'text-slate-500'}`}>
                      💻 Focus: {preset.focusMinutes} min | ☕ Pause: {preset.breakMinutes} min
                    </p>
                  </button>
                );
              })}
          ...
          <div className="card p-6 space-y-4">
            ...
            {uncompletedChapters.length === 0 ? (
              <p className={`text-[12px] italic ${isDark ? 'text-zinc-500' : 'text-slate-500'}`}>
                Aucun chapitre à réviser disponible. Créez-en un dans l'onglet "Chapitres".
              </p>
            ) : (
              <div className="space-y-3">
                <label className={`text-[12px] font-medium ${isDark ? 'text-zinc-400' : 'text-slate-600'}`}>
                ...
                {currentChapter && (
                  <div className={`p-3 rounded-xl border space-y-1 transition-colors ${
                    isDark ? 'bg-white/[0.02] border-zinc-800' : 'bg-slate-50 border-slate-200'
                  }`}>
                    <p className={`text-[10px] font-semibold uppercase tracking-wider ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>Chapitre actif</p>
                    <div className="flex items-center gap-2">
                      {currentSubject && <div className="w-2.5 h-2.5 rounded-full shrink-0 shadow-sm" style={{ backgroundColor: currentSubject.color }}></div>}
                      <span className={`text-[13px] font-bold truncate ${isDark ? 'text-zinc-200' : 'text-slate-800'}`}>{currentChapter.name}</span>
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="lg:col-span-2 card p-8 flex flex-col items-center justify-center relative overflow-hidden min-h-[450px]">
          ...
          <div className={`absolute w-80 h-80 rounded-full blur-3xl opacity-[0.15] transition-colors duration-1000 pointer-events-none ${
          ...
          <div className="relative z-10 flex flex-col items-center space-y-8">
            ...
            {/* Mode Indicator */}
            <div className={`flex items-center gap-2 px-5 py-2 rounded-full text-[13px] font-bold transition-colors duration-300 ${
              mode === 'focus' 
            ...
            <div className="relative w-72 h-72 flex items-center justify-center">
              ...
              <div className="absolute text-center space-y-1 flex flex-col items-center">
                <span className={`text-[56px] font-extrabold tracking-tighter tabular-nums leading-none ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  {formatTime(timeLeft)}
                </span>
                <p className={`text-[12px] uppercase tracking-[0.2em] font-bold ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>
            ...
            <div className="flex gap-5">
              ...
              <button
                ...
                className={`w-16 h-16 rounded-full flex items-center justify-center text-white transition-all duration-250 hover:scale-105 cursor-pointer shadow-[0_4px_16px_rgba(0,0,0,0.15)] ${
                  isRunning 
                    ? isDark ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-slate-800 hover:bg-slate-700'
                    : mode === 'focus'
            ...
            <div className={`flex items-center gap-1.5 text-[11px] font-semibold pt-2 ${isDark ? 'text-zinc-500' : 'text-slate-500'}`}>
