...
export const Stats = () => {
  const { subjects, chapters, studySessions, deleteStudySession, theme } =
    useAppStore();
  ...
  const handleDeleteSession = async (id) => {
    if (
      window.confirm(
        "Voulez-vous supprimer cette session d'étude des statistiques ?",
      )
    ) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div>
        ...
        >
          Analysez vos performances, votre répartition horaire et votre
          progression.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="card p-5 group">
          ...
          <div className="flex items-baseline gap-1.5">
            <h3
              className={`text-[28px] font-extrabold tracking-tight leading-none ${isDark ? "text-white" : "text-slate-900"}`}
            >
              {totalStudyHours}
            </h3>
            <span
              className={`text-[13px] font-semibold ${isDark ? "text-zinc-500" : "text-slate-400"}`}
            >
              h
            </span>
          </div>
          <p
            className={`text-[11px] font-medium mt-3 flex items-center gap-1.5 ${isDark ? "text-zinc-500" : "text-slate-500"}`}
          >
        ...
        <div className="card p-5 group">
          ...
          <p
            className={`text-[11px] font-medium mt-3 ${isDark ? "text-zinc-500" : "text-slate-500"}`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="card p-6 flex flex-col">
          <h2
            className={`text-[15px] font-bold mb-6 flex items-center gap-2 ${isDark ? "text-white" : "text-slate-900"}`}
          >
          ...
          <div className="flex-1 min-h-[260px] flex items-center justify-center">
            {pieChartData.length === 0 ? (
              <p
                className={`text-[13px] italic ${isDark ? "text-zinc-500" : "text-slate-400"}`}
              >
                Aucune donnée disponible. Lancez des sessions Focus.
              </p>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    ...
                    formatter={(value) => [`${value} minutes`, "Temps étudié"]}
                  />
                  ...
                  <Legend
                    ...
                    formatter={(value) => (
                      <span
                        className={`text-[12px] font-medium ml-1 ${isDark ? "text-zinc-400" : "text-slate-600"}`}
                      >
                        {value}
                      </span>
                    )}
            ...
            {barChartData.length === 0 ? (
              <div className="h-full flex items-center justify-center">
                <p
                  className={`text-[13px] italic ${isDark ? "text-zinc-500" : "text-slate-400"}`}
                >
                  Aucune donnée disponible. Créez des matières et des chapitres.
                </p>
              </div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="card p-6">
        ...
        >
          <History
            className={`w-[18px] h-[18px] ${isDark ? "text-zinc-400" : "text-slate-400"}`}
          />
        ...
        {recentSessionsList.length === 0 ? (
          <div className="py-10 text-center">
            <p
              className={`text-[13px] italic ${isDark ? "text-zinc-500" : "text-slate-400"}`}
            >
              Aucune session d'étude enregistrée dans l'historique.
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-[13px]">
              <thead
                className={`text-[11px] font-semibold uppercase tracking-wider border-b ${isDark ? "text-zinc-500 border-zinc-800" : "text-slate-400 border-slate-200"}`}
              >
                <tr>
              ...
              <tbody
                className={`divide-y font-medium ${isDark ? "divide-zinc-800/60" : "divide-slate-100"}`}
              >
                {recentSessionsList.map((session) => {
                  const chap = chapters.find(
                    (c) => c.id === session.chapter_id,
                  );
                  ...
                  return (
                    <tr
                      key={session.id}
                      className={`transition-colors ${isDark ? "hover:bg-zinc-900/30" : "hover:bg-slate-50/50"}`}
                    >
                      <td
                        className={`py-3.5 pr-4 whitespace-nowrap ${isDark ? "text-zinc-400" : "text-slate-500"}`}
                      >
                        {formattedDate}
                      </td>
                      <td className="py-3.5 pr-4 min-w-[200px]">
                        <div className="flex flex-col">
                          <span
                            className={`font-semibold truncate ${isDark ? "text-zinc-200" : "text-slate-800"}`}
                          >
